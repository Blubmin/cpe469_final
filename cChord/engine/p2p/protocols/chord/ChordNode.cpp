...
#define PERSISTENCE_DIR(a) ((a) + "/.chord/data")
#define PERSISTENCE_DIR_STRING(a) (PERSISTENCE_DIR(a) + "/%s")
...
void ChordNode::stabilize() {
	...
	if (notified && predecessor->getId() != thisNode->getId()) {
		...
		char path[256];
		DIR *dir = opendir(PERSISTENCE_DIR(root).c_str());
		if(dir != NULL){
			while(dirEntry=readdir(dir)) {
				if(dirEntry->d_type != DT_DIR){
					...
					if (!insideRange(hFilename, predecessor->getId(), thisNode->getId())){
						...
						sprintf(path, PERSISTENCE_DIR_STRING(root).c_str(), dirEntry->d_name);
						if( remove( path ) != 0 ) {
...
void ChordNode::saveData(string filename, string value){
	char path[256];
	sprintf(path, PERSISTENCE_DIR_STRING(root).c_str(), filename.c_str());
	cout << "Saving " << PERSISTENCE_DIR_STRING(root).c_str() << endl;
	mkdir(root.c_str(), 0777);
	mkdir((root + "/.chord/").c_str(), 0777);
	mkdir((root + "/.chord/data").c_str(), 0777);
...
void ChordNode::removekey(string key) {
	...
	if (insideRange(hKey, predecessor->getId() + 1, thisNode->getId())) {
		// I'm responsible for this key
		char path[256];
		sprintf(path, PERSISTENCE_DIR_STRING(root).c_str(), key.c_str());
		if( remove( path ) != 0 ) {
			perror( "Error deleting file" );
		}
	} else {
